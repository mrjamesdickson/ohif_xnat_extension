...
function DataSourceWrapper(props: withAppTypes) {
  ...
  const getInitialDataSourceName = useCallback(() => {
    ...
    console.log('getInitialDataSourceName: dataSourceName from URL:', dataSourceName);
    console.log('getInitialDataSourceName: window.config.defaultDataSourceName:', window.config.defaultDataSourceName);

    if (!dataSourceName && window.config.defaultDataSourceName) {
      console.log('getInitialDataSourceName: returning defaultDataSourceName');
    ...
    console.log('getInitialDataSourceName: returning dataSourceName:', dataSourceName);
  ...
  useEffect(() => {
    ...
    async function getData() {
      ...
      console.log('[XNAT] Wrapper fetching studies with filters:', queryFilterValues);
      const studies = await dataSource.query.studies.search(queryFilterValues);
      console.log('[XNAT] Wrapper received studies count:', studies?.length);
      ...
      if (studyUIDFilterRaw) {
        ...
        if (uidSet.size > 0) {
          ...
          filteredStudies = filteredStudies.filter(study => {
            ...
            if (!match) {
              console.log('[XNAT] Wrapper hiding study UID:', candidateUID);
            }
            return match;
          });
          console.log('[XNAT] Wrapper filtered studies count:', filteredStudies.length);
        }
      }
